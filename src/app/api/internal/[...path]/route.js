import { decryptHeader } from "@/utils/encryption";
...
const passthrough = async (req, method, path) => {
  ...
  const encryptedAuth = headers.get("x-encrypted-auth");
  if (encryptedAuth) {
    try {
      const decryptedAuth = decryptHeader(encryptedAuth);
      headers.set("Authorization", decryptedAuth);
      headers.delete("x-encrypted-auth");
    } catch (error) {
      console.error("Failed to decrypt auth header:", error);
